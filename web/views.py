...
def regist_submit(request):
    accout = request.POST.get('accout')     # 测试计划名
    passwd1 = request.POST.get('passwd1')       # 测试计划 id
    passwd2 = request.POST.get('passwd2')      # 执行次数
    if accout=='' or passwd1=='' or passwd2 =='' or passwd1!=passwd2:
        return HttpResponseRedirect('/regist_fail/')

    userinfo = models.UserInfo.objects.filter(name=accout, password=passwd1)
    userinfo.create(name=accout, password=passwd1)
    return HttpResponseRedirect('/regist_success/')

def regist_fail(request):
